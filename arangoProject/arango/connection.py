...
from .model import groups, mainGraph, members, users
from pyArango.theExceptions import *

vertexCollections = ['Users', 'Groups']
edgeCollections = ['Members']
GRAPH = 'mainGraph'
...
class arangoConnect():
    ...
    def __init__(self):
        ...
        if not self.db.hasCollection('Users') :
            self.userCollection = self.db.createCollection('Collection', "Users")

        if not self.db.hasCollection('Groups') :
            self.groupCollection = self.db.createCollection('Collection', "Groups")
        ...
        # 메인 Graph 생성 및 확인
        if not self.db.hasGraph('mainGraph') :
            self.mainGraph = self.db.createGraph(name="mainGraph")
    ...
    # 성공적으로 Vertex를 추가 하였을 경우 해당 vertex doc 리턴, 실패시 False리턴
    ...
    # 성공적으로 Relation(Edge)를 추가 하였을 경우 해당 relation doc 리턴, 실패시 False리턴
    ...
    def removeVertex(self, vertexCollection, vertexKey):
        try :
            graph = self.db.graphs[GRAPH].deleteVertex(self.db[vertexCollection][vertexKey])
        except Exception :
